...
def setup_mobile_io(m: 'MobileIO'):
    ...
    m.set_button_label(LIGHT_BTN, 'light')
    ...
    m.set_button_label(PROBE_BTN, 'probe')
    ...
    m.set_button_mode(DEPLOY_PROBE_BTN, 1)

    m.set_axis_label(PROBE_X_AXIS, 'probe ctrl')
    m.set_axis_label(PROBE_Y_AXIS, '')
...
if __name__ == '__main__':
    ...
    probe_pub = rospy.Publisher('/probe_pose_delta', Twist, queue_size=1)

    light_pub = rospy.Publisher('/pan_tilt_ctrl/light', Float64, queue_size=1)
    camera_goto = rospy.ServiceProxy('/pan_tilt_ctrl/goto_next', Trigger)
    ...
    while not rospy.is_shutdown():
        ...
        if mio.get_button_state(LIGHT_BTN):
            light_level.data = (mio.get_axis_state(LIGHT_AXIS) + 1.0) / 2.0
        else:
            light_level.data = 0.0

        if mio.get_button_state(GOTO_BTN):
            resp = camera_goto()
            if resp.success:
                mio.set_button_label(GOTO_BTN, resp.message, blocking=False)
        ...
        if mio.get_button_diff(DEPLOY_PROBE_BTN) == 1:
            deploy_probe(True)
        elif mio.get_button_diff(DEPLOY_PROBE_BTN) == -1:
            deploy_probe(False)
            probe_pose_delta.linear.x = 0.0
            probe_pose_delta.linear.y = 0.0
            probe_pose_delta.linear.z = 0.0
            mio.set_button_mode(PROBE_BTN, 0)
            mio.set_button_mode(PROBE_BTN, 1)
            probe_pose_delta.angular.z = -np.pi / 2.0

        if mio.get_button_diff(PROBE_BTN) == 1:
            mio.set_button_label(PROBE_BTN, 'nir')
        elif mio.get_button_diff(PROBE_BTN) == -1:
            mio.set_button_label(PROBE_BTN, 'probe')

        if mio.get_button_state(DEPLOY_PROBE_BTN) == 1:
            dx = 0.02 * mio.get_axis_state(PROBE_X_AXIS)
            dy = -0.02 * mio.get_axis_state(PROBE_Y_AXIS)
            dz = 0.05 * mio.get_axis_state(PROBE_Z_AXIS)

            probe_pose_delta.linear.x = dx 
            probe_pose_delta.linear.y = dy
            probe_pose_delta.linear.z = dz 

            using_probe = mio.get_button_state(PROBE_BTN)
            if using_probe:
                probe_pose_delta.angular.z = np.pi 
            else:
                probe_pose_delta.angular.z = np.pi/2.0
